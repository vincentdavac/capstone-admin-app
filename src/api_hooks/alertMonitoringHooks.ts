...
export const useAlertMonitor = (prototypeCode: string, pollInterval: number = 5000,prototypeId:string) => {
  ...
  const checkAlerts = async () => {
    try {
      ...
      console.log("hahhaha",buoyCode);
      
      const status = await alertMonitoring.checkAlertStatus(prototypeId,token ?? '');
      if (status.current_level === 'White') {
        setPreviousLevels({}); 
        setShownAlertIds(new Set());
        return;
      }
      const { alerts, has_new_alerts } = await alertMonitoring.getActiveAlerts(prototypeCode,token ?? '');
