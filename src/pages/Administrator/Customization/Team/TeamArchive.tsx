...
interface Props {
  show: boolean;
  onClose: () => void;
  TeamId: number | null; // ID of the team member to archive
  token: string;
  onArchived?: () => void;
  alertsRef: React.RefObject<AlertsContainerRef | null>;
}

const TeamArchive: React.FC<Props> = ({
  show,
  onClose,
  TeamId,
  token,
  ...
  if (!show || !TeamId) return null;

  const handleArchive = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/teams/${TeamId}?_method=PATCH`, {
        ...
        body: JSON.stringify({
          is_archived: true, // must be snake_case for backend
        }),
      });
      ...
      if (!res.ok) {
        ...
        if (!data.errors && !data.message) {
          alertsRef.current?.addAlert("error", "Failed to archive team.");
        }
      ...
      // SUCCESS
      alertsRef.current?.addAlert("success", "Team archived successfully!");
      onArchived?.();
      onClose();
    } catch (error: any) {
      ...
      alertsRef.current?.addAlert(
        "error",
        "An unexpected error occurred while archiving the team."
      );
    }
  };
      ...
      <div className="relative bg-white/90 dark:bg-gray-900/90 border border-white/20 rounded-2xl shadow-2xl w-full max-w-lg p-8 z-[10000] overflow-y-auto max-h-[90vh]">
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
            This will move the team member to the archive. You can restore them
            anytime.
          </p>
