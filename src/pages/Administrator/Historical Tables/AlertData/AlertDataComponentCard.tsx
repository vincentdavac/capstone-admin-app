...
const AlertDataComponentCard: React.FC<AlertDataComponentCardProps> = ({
  ...
  const fetchRelayStatuses = useCallback(async () => {
    if (!buoyId) return;
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/relay-statuses?${params.toString()}`,
        {
          headers: {
      ...
      if (res.ok && data.data) {
        setRelayStatuses(data.data);
      } else {
        setRelayStatuses([]);
        console.error("Failed to fetch relay statuses:", data);
      }
    } catch (error) {
      setRelayStatuses([]);
  ...
  const generateReport = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/relay-status-report?${params.toString()}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      link.href = url;
      link.setAttribute("download", `alert_data_report_${buoycode}.pdf`);
    ...
    >
      ...
      <div className="p-4 border-t border-gray-100 dark:border-gray-800 sm:p-6">
        {loading ? (
          ...
          <>
            <AlertDataChart relayStatuses={relayStatuses} />
            <AlertDataTable relayStatuses={relayStatuses} />
          </>
        )}
      </div>
    </div>
