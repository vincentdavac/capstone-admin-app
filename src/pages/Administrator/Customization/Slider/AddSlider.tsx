...
const AddSlider = ({ show, onClose, token, alertsRef, onAdded }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        alertsRef.current?.addAlert(
          "success",
          "Homepage slider added successfully!"
        );
        onAdded();
        onClose();
      } else {
        ...
        if (result.errors) {
          ...
          alertsRef.current?.addAlert(
            "error",
            result.message || "Failed to add slider."
          );
        }
      }
    } catch (err: any) {
      console.error("Error:", err);
      alertsRef.current?.addAlert("error", "Error adding slider.");
    }
  };
