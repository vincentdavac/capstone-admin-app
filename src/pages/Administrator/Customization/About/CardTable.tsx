...
import ArchiveSlider from "./CardArchive";
import UpdateSlider from "./CardUpdate";
import AddSlider from "./CardAdd";
...
const CardTable = ({ alertsRef, onRefresh }: Props) => {
  ...
  const fetchCards = async () => {
    ...
    try {
      ...
      if (res.ok && data.data) {
        ...
        console.error("Failed to fetch about cards:", data);
      }
    } catch (error) {
  ...
  const filteredCards = cards.filter((c) => {
    ...
    const { cardTitle, cardDescription, createdDate } = c.attributes;
    return (
      cardTitle.toLowerCase().includes(term) ||
      cardDescription.toLowerCase().includes(term) ||
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full border border-gray-200 dark:border-gray-700 rounded-lg">
            <thead className="bg-gray-50 dark:bg-gray-700">
              <tr>
                {["No.", "Title", "Description", "Status", "Actions"].map(
                  (h) => (
                    <th
                      key={h}
