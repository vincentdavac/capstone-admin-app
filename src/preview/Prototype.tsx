...
export default function Prototype() {
  ...
  return (
    <section className="w-full bg-white py-16 outline-1 outline-[#959ea5] rounded-lg dark:bg-gray-900 transition-colors duration-300">
      <div className="container mx-auto px-4">
        <div className="mb-12 text-center">
          ...
          <p className="max-w-8xl mx-auto pt-5 text-center leading-relaxed text-[#023E8A] dark:text-gray-300 sm:text-xl md:text-xl transition-colors duration-300">
        ...
        <div className="relative flex flex-col items-center gap-8 md:flex-row md:justify-center md:gap-12">
          ...
          <div className="hidden flex-col justify-center gap-6 md:flex">
            {leftImages.map((item, idx) => (
              <div
                key={idx}
                className="group relative flex h-25 w-44 items-center justify-center rounded-xl border border-[#023E8A] bg-white shadow-md transition-all duration-300 hover:scale-105 hover:shadow-xl dark:border-blue-400 dark:bg-gray-800 dark:shadow-lg dark:hover:shadow-xl"
              ...
              >
                ...
                <div
                  className={`absolute bottom-full left-1/2 z-50 mb-3 w-64 -translate-x-1/2 transform rounded-lg bg-[#023E8A] p-4 text-white shadow-xl transition-all duration-300 dark:bg-blue-600 ${
          ...
          <div className="z-10 flex w-full max-w-sm items-center justify-center rounded-xl border border-[#023E8A] bg-white p-4 shadow-lg sm:max-w-md md:h-[500px] md:w-[400px] dark:border-blue-400 dark:bg-gray-800">
            <ModelViewer />
          </div>
        ...
        <div className="mt-10 grid grid-cols-2 gap-4 sm:grid-cols-4 md:hidden">
          {[...leftImages, ...rightImages].map((item, idx) => (
            <div
              key={idx}
              className="group relative flex h-24 w-full items-center justify-center rounded-xl border border-gray-200 bg-white shadow-md dark:border-gray-700 dark:bg-gray-800"
            >
              <img
                src={item.src}
                alt={item.title}
              ...
              <div className="invisible absolute -bottom-2 left-1/2 z-50 w-48 -translate-x-1/2 -translate-y-2 translate-y-full transform rounded-lg bg-[#023E8A] p-3 text-white opacity-0 shadow-xl transition-all duration-300 group-hover:visible group-hover:translate-y-full group-hover:opacity-100 dark:bg-blue-600">
                <h3 className="mb-1 text-sm font-bold">{item.title}</h3>
                <p className="text-xs">{item.description}</p>
                <div className="absolute -top-2 left-1/2 h-0 w-0 -translate-x-1/2 transform border-t-0 border-r-8 border-b-8 border-l-8 border-r-transparent border-b-[#023E8A] dark:border-b-blue-600 border-l-transparent"></div>
              </div>
            </div>
          ))}
        </div>
