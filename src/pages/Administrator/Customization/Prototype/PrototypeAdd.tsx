...
const PrototypeAdd = ({ show, onClose, token, alertsRef, onAdded }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        alertsRef.current?.addAlert("success", "Prototype added successfully!");
        onAdded();
        onClose();
      } else {
        ...
        // Then show general message if exists
        if (result.message) {
        ...
        if (!result.errors && !result.message) {
          alertsRef.current?.addAlert("error", "Failed to add prototype.");
        }
      }
    } catch (err: any) {
      console.error("Error:", err);
      alertsRef.current?.addAlert("error", "Error adding prototype.");
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <select
              ...
              className="w-full p-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 transition"
            >
