  ...
  const fetchBatteryHealth = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/battery-health?${params.toString()}`,
        {
          headers: {
      ...
      const data: BatteryHealthResponse = await res.json();

      if (res.ok && data.data) {
        setBatteryHealth(data.data);
      } else {
        setBatteryHealth([]);
        console.error("Failed to fetch battery health:", data);
      }
    } catch (error) {
      setBatteryHealth([]);
    ...
    >
      ...
      <div className="flex flex-col gap-4 px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
        <h3 className="text-base font-medium text-gray-800 dark:text-white/90">
          Battery Health Historical Data
        </h3>
      ...
      <div className="p-4 border-t border-gray-100 dark:border-gray-800 sm:p-6">
        {loading ? (
          ...
          <>
            <BatteryHeathChart batteryHealth={batteryHealth} />
            <BatteryHealthTable batteryHealth={batteryHealth} />
          </>
        )}
      </div>
    </div>
