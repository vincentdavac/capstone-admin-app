...
interface Props {
  show: boolean;
  onClose: () => void;
  FeedbackId: number | null; // ID of the feedback to archive
  token: string;
  onArchived?: () => void;
  alertsRef: React.RefObject<AlertsContainerRef | null>;
}

const FeedbackArchive: React.FC<Props> = ({
  show,
  onClose,
  FeedbackId,
  token,
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (!data.errors && !data.message) {
          alertsRef.current?.addAlert("error", "Failed to archive feedback.");
        }
      ...
      // SUCCESS
      alertsRef.current?.addAlert("success", "Feedback archived successfully!");
      onArchived?.();
      onClose();
    } catch (error: any) {
      ...
      alertsRef.current?.addAlert(
        "error",
        "An unexpected error occurred while archiving the feedback."
      );
    }
  };
      ...
      <div className="relative bg-white/90 dark:bg-gray-900/90 border border-white/20 rounded-2xl shadow-2xl w-full max-w-lg p-8 z-[10000] overflow-y-auto max-h-[90vh]">
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
            This will move the feedback to the archive. You can restore it
            anytime.
          </p>
